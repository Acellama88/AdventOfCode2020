    ...
    public partial class Form1 : Form
    {
        ...
        public Form1()
        {
            ...
            foreach (string val in values)
                report.Add(Convert.ToInt32(val));
            report.Sort();
        }

        private void button1_Click(object sender, EventArgs e)
        {
            int length = report.Count;
            int goal = 2020;
            bool done = false;
            for (int c1 = 0; !done && c1 < length; c1++)
            {
                for (int c2 = c1 + 1; !done && c2 < length; c2++)
                {
                    int total = report[c1] + report[c2];
                    if (total == goal)
                    {
                        MessageBox.Show("" + report[c1] * report[c2]);
                        done = true;
                    }
                }
            }
        ...
        private void button2_Click(object sender, EventArgs e)
        {
            ...
            for (int c1 = 0; !done && c1 < length; c1++)
            {
                for (int c2 = c1 + 1; !done && c2 < length; c2++)
                {
                    for (int c3 = c2 + 1; !done && c3 < length; c3++)
                    {
                        int total = report[c1] + report[c2] + report[c3];
                        if (total == goal)
                        {
                            MessageBox.Show("" + report[c1] * report[c2] * report[c3]);
                            done = true;
                        }
                    }
                }
